...
class FavoriteEditText : androidx.appcompat.widget.AppCompatEditText {
    ...
    override fun draw(canvas: Canvas) {
        ...
        //显示文本长度
        canvas.drawText(hintString, -offsetXHint + (width - hintWidth) - 8f.dp2px, hintBaseline, paint)
        //清空按钮
        canvas.drawBitmap(clearBitmap, offsetXClear + width - clearBitmap.width - 8f.dp2px, (height - clearBitmap.height) / 2f, Paint())
        canvas.translate((-scrollX).toFloat(), 0f)
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        if (event?.action == MotionEvent.ACTION_DOWN
                && x >= width - clearBitmap.width - 8f.dp2px
                && x <= width - clearBitmap.width - 8f.dp2px + clearBitmap.width

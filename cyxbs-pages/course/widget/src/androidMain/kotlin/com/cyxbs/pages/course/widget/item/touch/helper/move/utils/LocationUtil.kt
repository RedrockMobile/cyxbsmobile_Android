package com.cyxbs.pages.course.widget.item.touch.helper.move.utils
...
object LocationUtil {
   ...
   * 得到移动后的位置
   *
   * @param isSkipForeachJudge 是否跳过循环。如果返回 true，则允许发生重叠
   *
  ...
  fun getLocation(
    ...
    * 原因如下：
    * 虽然目前课表的每个 item 宽高都是 match，但存在 View 设置了 margin，
    * 并且后面会使用这个值获取行和列，所以不使用 View 的位置来判断
    * */
    ...
    * 如果不修正，那由边界高度值取得的行数和列数可能将比总行数和总列数大上一行或一列
    * */
    ...
    * */
    ...
    /*
    * 第二次遍历：
    * 1、对于修正后的最终位置再次遍历子 View，寻找是否与其他子 View 有交集，若有，则回到原位置
    * */
    for (i in 0 until parent.getChildCount()) {
      val child = parent.getChildAt(i)
      if (child === view) continue

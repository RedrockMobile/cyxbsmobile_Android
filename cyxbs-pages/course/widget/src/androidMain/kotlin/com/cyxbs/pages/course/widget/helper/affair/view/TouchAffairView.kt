package com.cyxbs.pages.course.widget.helper.affair.view
...
import com.cyxbs.pages.course.widget.item.helper.expand.DoubleSideExpandableHelper
...
open class TouchAffairView(context: Context) : ItemView(context), ITouchAffairItem {
  ...
  protected val mImageView = ImageView(context).apply {
    ...
    // 设置 ImageView 的前景图片
    setImageResource(R.drawable.course_widget_ic_touch_affair)
  }
  
  init {
    super.setCardBackgroundColor(R.color.course_widget_affair_bg.color)
    super.addView(
      mImageView,
  ...
  override val lp = SingleDayLayoutParams(-1, -1, 0).apply {
    leftMargin = R.dimen.course_widget_item_margin.dimen.toInt() // 课表中 item 的边距值

...
import com.mredrock.cyxbs.lib.base.crash.CrashActivity.Companion.NetworkApiResult
...
object CrashMonitor {
  ...
  private fun handleOtherThread(thread: Thread, throwable: Throwable) {
    CrashDialog.Builder(RuntimeException("触发了一次来自其他线程的异常, thread.name: ${thread.name}, " +
        "message: ${throwable.message}", throwable)).show()
  }

  private fun handleMainThread(thread: Thread, throwable: Throwable) {
    CrashDialog.Builder(RuntimeException("触发了一次来自主线程的异常, ${throwable.message}", throwable)).show()
